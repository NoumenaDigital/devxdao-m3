...
use crate::utility::constants::{
    ARG_ACCOUNT_WHITELIST, ARG_CONTRACT_WHITELIST, ARG_HOLDER_MODE, ARG_IDENTIFIER_MODE,
    ARG_METADATA_MUTABILITY, ARG_WHITELIST_MODE,
};
...
impl InstallerRequestBuilder {
    ...
    pub(crate) fn default() -> Self {
        InstallerRequestBuilder {
            ...
            account_whitelist: CLValue::from_t(Some(Vec::<AccountHash>::new())).unwrap(),
    ...
    pub(crate) fn with_account_whitelist(mut self, account_whitelist: Vec<AccountHash>) -> Self {
        self.account_whitelist = CLValue::from_t(Some(account_whitelist)).unwrap();
        self
    }
