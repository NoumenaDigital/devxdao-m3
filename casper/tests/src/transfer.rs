...
use crate::utility::{
    constants::{
        ...
        ARG_SOURCE_KEY, ARG_TARGET_KEY, ARG_TOKEN_HASH, ARG_TOKEN_ID, ARG_TOKEN_ID_HASH, ARG_TOKEN_META_DATA,
        ...
        TEST_PRETTY_721_META_DATA, TEST_TOKEN_ID_HASH_DATA, TOKEN_OWNERS, TRANSFER_SESSION_WASM,
    },
    ...
    let mint_session_call = ExecuteRequestBuilder::standard(
        *DEFAULT_ACCOUNT_ADDR,
        ...
        runtime_args! {
            ...
            ARG_TOKEN_ID_HASH => TEST_TOKEN_ID_HASH_DATA.to_string(),
        },
    )
    .build();
...
fn should_transfer_token_from_sender_to_receiver() {
    ...
    let install_request = InstallerRequestBuilder::new(*DEFAULT_ACCOUNT_ADDR, NFT_CONTRACT_WASM)
        ...
        .with_account_whitelist(vec![
            ...
            token_receiver.to_account_hash(),
        ])
        .build();
...
fn should_be_able_to_transfer_token_using_approved_operator() {
    ...
    let install_request = InstallerRequestBuilder::new(*DEFAULT_ACCOUNT_ADDR, NFT_CONTRACT_WASM)
        ...
        .with_account_whitelist(vec![
            DEFAULT_ACCOUNT_PUBLIC_KEY.clone().to_account_hash(),
            to_account_public_key.to_account_hash(),
        ])
        .build();
...
#[test]
fn should_disallow_same_operator_to_tranfer_token_twice() {
    let (_, to_account_public_key) = support::create_dummy_key_pair(ACCOUNT_USER_2);
    ...
    let install_request = InstallerRequestBuilder::new(*DEFAULT_ACCOUNT_ADDR, NFT_CONTRACT_WASM)
        ...
        .with_account_whitelist(vec![
            DEFAULT_ACCOUNT_PUBLIC_KEY.clone().to_account_hash(),
            to_account_public_key.to_account_hash(),
            to_other_account_public_key.to_account_hash(),
        ])
        .build();
...
fn should_transfer_between_contract_to_account() {
    ...
    let install_request = InstallerRequestBuilder::new(*DEFAULT_ACCOUNT_ADDR, NFT_CONTRACT_WASM)
        ...
        .with_account_whitelist(vec![DEFAULT_ACCOUNT_PUBLIC_KEY.clone().to_account_hash()])
        .build();
    ...
    let transfer_request = ExecuteRequestBuilder::standard(
        *DEFAULT_ACCOUNT_ADDR,
        ...
        runtime_args! {
            ...
            ARG_TOKEN_HASH => TEST_TOKEN_ID_HASH_DATA.to_string(),
            ARG_SOURCE_KEY => Key::Account(*DEFAULT_ACCOUNT_ADDR),
