...
use crate::utility::{
    constants::{
        ...
        ARG_OPERATOR, ARG_TOKEN_ID, ARG_TOKEN_ID_HASH, ARG_TOKEN_META_DATA, ARG_TOKEN_OWNER, BALANCES,
...
fn should_disallow_public_minting_with_flag_set_to_false() {
    ...
    assert_expected_error(
        error,
        36,
        "only contract installer can mint when MintingMode.Installer is set",
    );
}
...
fn should_mint_with_hash_identifier_mode() {
    ...
    assert_eq!(vec![TEST_TOKEN_ID_HASH_DATA.to_string()], actual_token_ids);
}
