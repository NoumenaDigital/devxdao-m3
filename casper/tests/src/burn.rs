...
use crate::utility::{
    constants::{
        ...
        OWNED_TOKENS_DICTIONARY_KEY, TEST_PRETTY_721_META_DATA, TEST_TOKEN_ID_HASH_DATA, TOKEN_COUNTS,
        ARG_TOKEN_ID_HASH,
    },
...
#[test]
fn should_return_expected_error_burning_of_others_users_token_only_contract_creator_can_burn() {
    let mut builder = InMemoryWasmTestBuilder::default();
    builder.run_genesis(&DEFAULT_RUN_GENESIS_REQUEST).commit();
    ...
    support::assert_expected_error(error, 1u16, "should disallow burning of other users' token");
}

#[test]
fn should_return_expected_error_when_burning_not_owned_token_only_contract_creator_can_burn() {
    let mut builder = InMemoryWasmTestBuilder::default();
    builder.run_genesis(&DEFAULT_RUN_GENESIS_REQUEST).commit();
