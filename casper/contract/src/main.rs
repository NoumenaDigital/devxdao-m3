...
fn enforce_only_contract_creator_can_invoke() {
...
pub extern "C" fn init() {
    ...
    let account_whitelist = get_named_arg_with_user_errors::<Vec<AccountHash>>(
    ...
    let metadata_mutability: MetadataMutability = get_named_arg_with_user_errors::<u8>(
...
#[no_mangle]
pub extern "C" fn add_to_account_whitelist() {
    let installer = get_account_hash(
        INSTALLER,
        NFTCoreError::MissingInstaller,
        NFTCoreError::InvalidInstaller,
    ...
    let new_account_for_whitelist_key = get_named_arg_with_user_errors::<Key>(
        ARG_ACCOUNT_WHITELIST,
        NFTCoreError::MissingAccountWhiteList,
        NFTCoreError::InvalidAccountWhitelist,
    )
    ...
    let mut existing_account_whitelist = get_stored_value_with_user_errors::<Vec<AccountHash>>(
        ACCOUNT_WHITELIST,
        NFTCoreError::MissingAccountWhiteList,
        NFTCoreError::InvalidAccountWhitelist,
    );

    let new_account_for_whitelist = new_account_for_whitelist_key
        .into_account()
        .unwrap_or_revert_with(NFTCoreError::InvalidKey);

    existing_account_whitelist.push(new_account_for_whitelist.clone());
    ...
    storage::write(whitelist_uref, existing_account_whitelist)
}

#[no_mangle]
pub extern "C" fn remove_from_account_whitelist() {
    let installer = get_account_hash(
        INSTALLER,
        NFTCoreError::MissingInstaller,
        NFTCoreError::InvalidInstaller,
    ...
    let account_to_remove_from_whitelist_key = get_named_arg_with_user_errors::<Key>(
        ARG_ACCOUNT_WHITELIST,
        NFTCoreError::MissingAccountWhiteList,
        NFTCoreError::InvalidAccountWhitelist,
    )
    ...
    let account_to_remove_from_whitelist = account_to_remove_from_whitelist_key
        .into_account()
        .unwrap_or_revert_with(NFTCoreError::InvalidKey);

    let index = existing_account_whitelist
        .iter()
        .position(|item| *item == account_to_remove_from_whitelist)
        .unwrap();
    existing_account_whitelist.remove(index);
...
pub extern "C" fn mint() {
    ...
    let token_identifier: TokenIdentifier = match identifier_mode {
        ...
        NFTIdentifierMode::Hash => {
            TokenIdentifier::Hash(runtime::get_named_arg(ARG_TOKEN_ID_HASH))
        }
    };
...
pub extern "C" fn burn() {
    enforce_only_contract_creator_can_invoke();
...
pub extern "C" fn transfer() {
    ...
    let contract_installer_key = Key::from(installing_account);

    // Throws an error if:
    // holder_mode is Accounts AND caller is neither of
    // the owner, an approved operator (on-behalf-of), or the contract installer
    if NFTHolderMode::Accounts == holder_mode {
        if caller == token_owner_key || is_approved || caller == contract_installer_key {
            // we're fine
        } else {
            runtime::revert(NFTCoreError::InvalidAccount);
        }
    ...
    // check against account_whitelist
    match target_owner_key.tag() {
        KeyTag::Hash => {
            // runtime::revert(NFTCoreError::InvalidAccount);
            let calling_account = target_owner_key
                .into_hash()
                .map(AccountHash::new)
                .unwrap_or_revert_with(NFTCoreError::InvalidKey);
            ...
            // Revert if the calling contract is not in the whitelist.
            if !account_whitelist.contains(&calling_account) {
                runtime::revert(NFTCoreError::UnlistedAccountHash)
            }
        }
        KeyTag::Account => {
            // runtime::revert(NFTCoreError::MissingInstaller);
            let calling_account = target_owner_key
                .into_account()
                .unwrap_or_revert_with(NFTCoreError::InvalidKey);
            let account_whitelist = get_stored_value_with_user_errors::<Vec<AccountHash>>(
...
pub extern "C" fn owner_of() {
    ...
    let token_owner = match maybe_token_owner {
        Some(token_owner) => token_owner,
        None => runtime::revert(NFTCoreError::InvalidTokenIdentifier), /* If a token does not
                                                                        * have an
                                                                        * owner it could not have
                                                                        * been
                                                                        * minted. */
    };
...
pub extern "C" fn set_token_metadata() {
    ...
    let metadata_mutability: MetadataMutability = get_stored_value_with_user_errors::<u8>(
        METADATA_MUTABILITY,
        NFTCoreError::MissingMetadataMutability,
        NFTCoreError::InvalidMetadataMutability,
    )
    ...
    let token_owner = get_dictionary_value_from_key::<Key>(
        TOKEN_OWNERS,
        &token_identifier.get_dictionary_item_key(),
    );
    ...
    let updated_token_metadata: String = get_named_arg_with_user_errors::<String>(
        ARG_TOKEN_META_DATA,
        NFTCoreError::MissingTokenMetaData,
        NFTCoreError::InvalidTokenMetaData,
    )
    ...
    let updated_metadata =
        validate_metadata(&metadata_kind, updated_token_metadata).unwrap_or_revert();
    ...
    let entry_points = {
        ...
        let set_variables = EntryPoint::new(
            ...
            vec![
                ...
                Parameter::new(ARG_ACCOUNT_WHITELIST, CLType::Key),
            ],
            CLType::Unit,
            EntryPointAccess::Public,
            EntryPointType::Contract,
        ...
        // Account Whitelist
        let add_to_account_whitelist = EntryPoint::new(
            ENTRY_POINT_ADD_TO_ACCOUNT_WHITELIST,
            vec![Parameter::new(ARG_ACCOUNT_WHITELIST, CLType::Key)],
            CLType::Unit,
            EntryPointAccess::Public,
            EntryPointType::Contract,
        );

        let remove_from_account_whitelist = EntryPoint::new(
        ...
        let mint = EntryPoint::new(
            ...
            vec![
                ...
                Parameter::new(ARG_TOKEN_ID_HASH, CLType::String),
            ],
            CLType::Tuple3([
        ...
        entry_points.add_entry_point(add_to_account_whitelist);
        entry_points.add_entry_point(remove_from_account_whitelist);
...
pub extern "C" fn call() {
    ...
    let holder_mode: u8 =
        get_optional_named_arg_with_user_errors(ARG_HOLDER_MODE, NFTCoreError::InvalidHolderMode)
            .unwrap_or(0u8); // default to NFTHolderMode.Accounts if not specified
    ...
    let account_white_list: Vec<AccountHash> = get_optional_named_arg_with_user_errors(
        ARG_ACCOUNT_WHITELIST,
        NFTCoreError::InvalidAccountWhitelist,
    )
    .unwrap_or_default();
    ...
    // This value cannot be changed after installation.
    let metadata_mutability: u8 = get_named_arg_with_user_errors(
        ARG_METADATA_MUTABILITY,
        NFTCoreError::MissingMetadataMutability,
        NFTCoreError::InvalidMetadataMutability,
    )
